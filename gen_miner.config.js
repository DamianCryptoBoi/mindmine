...
// Dynamically load one miner instance's environment file.
const requestedEnvFile = process.env.GEN_MINER_ENV_FILE || '.env.gen_miner';
const envPath = path.isAbsolute(requestedEnvFile)
  ? requestedEnvFile
...
const pm2Name = process.env.MINER_PM2_NAME || 'bitmind-generative-miner';
const config = {
  // Wallet
  ...
  taskTimeout: process.env.MINER_TASK_TIMEOUT || '1800',
...
const apps = [
  {
    ...
    env: {
      ...DYNAMIC_ENV,
      GEN_MINER_ENV_FILE: envPath,
      MINER_PM2_NAME: pm2Name,
    },
    watch: false,
    instances: 1,
    autorestart: true,
