...
def test_default_retries_stop_after_three():
    assert push_model.should_retry_register(1, 3) is True
    assert push_model.should_retry_register(2, 3) is True
    assert push_model.should_retry_register(3, 3) is False


def test_zero_retries_is_unlimited():
    assert push_model.should_retry_register(1, 0) is True
    assert push_model.should_retry_register(99, 0) is True


def test_already_uploaded_is_success():
    result = {"already_uploaded": True, "success": False}
    assert push_model._accept_already_uploaded("image", result) is True
...
def test_chain_registration_failure_does_not_fail_push(monkeypatch, tmp_path):
    ...
    success = asyncio.run(
        push_model.push_separate_models(
