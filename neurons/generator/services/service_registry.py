...
from .gpti2_service import GPTi2Service
...
class ServiceRegistry:
    ...
    Set per-modality service via env vars:
      IMAGE_SERVICE=openai|openrouter|stabilityai|maxcheapai|ckey|vertexai|vertexgen|gpti2|none
    ...
    def _init_modality_services(self, image_service: str, video_service: str):
        ...
        if video_service == "none":
            ...
            if video_service in initialized:
                ...
                self.services["video"] = self.services.get(
                    "image"
    ...
    def _create_service(
        self, service_name: str, modality: str
        ...
        try:
            ...
            if service.is_available():
                ...
                bt.logging.error(
                    f"❌ {service.name} configured for {modality} but not available (check API keys)"
                )
    ...
    def get_all_api_key_requirements(self) -> Dict[str, str]:
        ...
        all_requirements = {
            "IMAGE_SERVICE": (
                "Service for images: openai, openrouter, stabilityai, maxcheapai, "
                "ckey, vertexai, vertexgen, gpti2, or none"
            ),
            "VIDEO_SERVICE": (
                "Service for videos: openai, openrouter, stabilityai, runway, "
                "maxcheapai, or none"
            ),
        }
