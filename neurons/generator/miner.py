...
from neurons.generator.task_checkpoint_store import (
    TaskCheckpointStore,
    resolve_state_dir,
)
...
class GenerativeMiner(BaseNeuron):
    ...
    def _start_background_workers(self):
        ...
        max_concurrent = max(
            1, int(getattr(self.config.miner, "max_concurrent_tasks", 5))
        )
        worker_count = max(1, int(getattr(self.config.miner, "worker_threads", 2)))
        worker_count = min(worker_count, max_concurrent)
        self.task_processors = []
        for index in range(worker_count):
            worker = threading.Thread(
                target=self._task_processing_loop,
                name=f"generation-worker-{index + 1}",
                daemon=True,
            )
            worker.start()
            self.task_processors.append(worker)
        bt.logging.info(f"Started {worker_count} generation worker threads")
    ...
    def _task_processing_loop(self):
        ...
        while True:
            try:
                task = self.task_manager.claim_next_pending_task(
                    lambda claimed: self._persist_task_snapshot(claimed.task_id)
                )
                if task is None:
    ...
    async def _process_task(self, task):
        ...
        resume = task.status == TaskStatus.PROCESSING and task.checkpoint is not None
        if resume:
            bt.logging.info(
        ...
        try:
            ...
            bt.logging.info(
                f"Using service: {service.__class__.__name__ if service else 'None'} for {task.modality}"
            )
            if not service:
    ...
    async def _call_service_process(self, service, task):
        ...
        try:
            ...
            if "data" not in result:
                bt.logging.error(
                    f"Service {service.__class__.__name__} result missing 'data' field"
                )
            ...
            if result_data is None:
                bt.logging.error(
                    f"Service {service.__class__.__name__} returned None for data field"
                )
                raise ValueError("Service result data field is None")

            bt.logging.info(
                f"✅ Service {service.__class__.__name__} completed successfully"
            )
            return result
    ...
    def _log_available_services(self):
        ...
        bt.logging.info("=" * 60)
        bt.logging.info(
            f"📊 Summary: {available_count}/{total_endpoints} endpoints functional"
        )

        if available_count == 0:
            bt.logging.warning(
                "⚠️  No services available! Miner will reject all requests."
            )
    ...
    def _save_to_output_dir(self, task, data: bytes):
        ...
        save_locally = os.getenv("MINER_SAVE_LOCALLY", "false").lower() in (
            "true",
            "1",
            "yes",
        )
    ...
    async def _handle_generation_request(self, request: Request, modality: str):
        ...
        try:
            ...
            try:
                ...
                bt.logging.debug(
                    f"Request body keys: {list(body.keys()) if body else 'None'}"
                )
            except Exception as e:
                bt.logging.error(f"Failed to parse JSON body: {e}")
                return self._error_response(f"Invalid JSON body: {str(e)}", 400)
            ...
            if not webhook_url:
                bt.logging.error(
                    f"Missing X-Webhook-URL header. Headers: {dict(request.headers)}"
                )
            ...
            if not service:
                bt.logging.warning(
                    f"Rejecting {modality} request: no service available"
                )
                return self._error_response(
        ...
        return self._success_response(
            {
                ...
                "config": {
                    ...
                    "task_timeout": getattr(self.config.miner, "task_timeout", 1800),
