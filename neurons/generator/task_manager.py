...
class TaskManager:
    ...
    def set_checkpoint(
        self, task_id: str, checkpoint: Optional[Dict[str, Any]]
    ) -> bool:
        with self._lock:
    ...
    def claim_next_pending_task(
        self, on_claim: Optional[Callable[[GenerationTask], None]] = None
    ) -> Optional[GenerationTask]:
        """Claim one task, rolling it back if its pre-processing hook fails."""
        claimed = None
        with self._lock:
            for task in self.tasks.values():
                if task.status == TaskStatus.PENDING:
        ...
        try:
            ...
            with self._lock:
                current = self.tasks.get(claimed.task_id)
                if current is claimed and current.status == TaskStatus.PROCESSING:
